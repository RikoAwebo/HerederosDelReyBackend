    ...
    public class DetalleVentaController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            if (detalleVenta == null)
                return NotFound(new { mensaje = "detalleventa no encontrado" });

            return Ok(detalleVenta);
        }

        [HttpPost]
        public async Task<IActionResult> Create(DetalleVentaCreateDto dto)
        {
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, DetalleVentaUpdateDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            ...
            if (!ok)
                return NotFound(new { mensaje = "detalleventa no encontrada" });
